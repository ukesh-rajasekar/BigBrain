...
const fetchGames = () => {
  ...
  return (
    <React.Fragment>
      ...
      <div>
        {newGame && <Button buttonText="Submit" buttonAction={oncreate} />}
      </div>
      <div>
        <div className="gamesWrapper">
          <div className="gamesContainer">
            {Object.entries(games).map((value) => {
              return (
                <React.Fragment key={value[1].id}>
                  <h3 key={value[0]}>{value[1].name}</h3>
                  <div>
                    {!sessionStatus && (
                      <Button
                        buttonText="Start game"
                        buttonAction={() => onstart(value[1].id)}
                      />
                    )}
                  </div>
                  <div>
